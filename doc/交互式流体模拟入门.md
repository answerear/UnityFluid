...
SPH 开始是用来求天体物理的。它有什么好处呢？它有一个最大的好处就是不需要 mesh，非常适合用来模拟自由表面的流体。什么叫自由表面的流体呢？比如说你有一盆水，然后这个水有一个水和空气之间的接触面，那什么不是自由表面的流体呢，比说烟雾就不是自由表面的流体了。

SPH 有个很大的好处就是，直观上非常容易理解，你可以理解成每个粒子是一小滴水。另外，作为粒子法，它还有一个更大的好处就是，天然支持并行。
...
有了密度以后，我们就可以估算压力了。这里我们借用理想气体状态方程：
$$
p=k\rho
\label{eq:NS-Lagrange-Ideal-Gas-State-Equation}
$$
引用到流体力，通过密度来近似获得压力，即：
$$
p_i = k(\rho_i - \rho_0)
...
一般用得比较多的是 poly6 和 spiky。当然也有人用三次样条，反正只要是一种权重插值函数就可以了，SPH 也没有具体的限制。这里我们简单讨论下 poly6 和 spiky。
...
举个简单例子，如上图，我们讲空间划分将空间划分成大小为 $h$ 的单元，这样子只需要搜索 27 个单元格。大概步骤如下：
...
举个简单例子，比较直观地看怎么做位置修正的。一个粒子在圆环轨道上运动，如图所示：
...
首先我们把高斯最小二乘约束原理的最小二乘法改一下形式，因为这里我们不需要求最小值，而是要求满足最小值的参数 $\ddot{\vec{p}}$，所以，我们可以写成：
$$
...
三个等式约束，但是无法同时满足这三个约束，所以是无解的。那是不是就无法去求解呢？在 PBD 中，直接忽略这种有无解的情况，直接去求解。我们看下 PBD 可以用什么方法求解，并且相关的方法的优劣。直接求解方程组，一般由于矩阵逆是否存在的问题，所以可能导致无法求解，所以一般我们都用迭代法来代替直接求解。在 PBD 中的迭代求解一般有以下两种：
...
使用高斯-赛德尔方法求解，第一步先代入 约束 $C_1$ ，解会在 $C_1$ 的解空间 $l_1$ 中。然后把解代入约束 $C_2$ ，解会在约束 $C_2$ 的解空间 $l_2$ 中。也就是说用高斯-赛德尔方法解会在解空间之间来回跳跃，然后慢慢靠近共同的解空间。整体来讲，高斯-赛德尔迭代收敛速度较慢，并且由于每一个约束方程求解都需要用上一个约束方程的解作为当前约束方程的输入，所以导致无法并行。
...
雅可比迭代先求解关于约束 $C_1$ 的方程，计算出到 $C_1$ 解空间 $l_1$ 的向量。然后求解关于约束 $C_2$ 的方程到 $C_2$ 在解空间 $l_2$ 的向量。然后向着两个向量的合向量前进，寻找关于 $C_1$ 、$C_2$ 的相交解空间。这样的话如果 $C_1$ 和 $C_2$ 的解空间在同一个方向，雅可比迭代使用合向量作为步长，经常会一步迈过，下一步再买回起始点，导致不能收敛。但是当解空间不同向的时候，雅可比迭代法由于向着合向量迭代，不会像高斯-赛德尔方法那样在两个解空间之间来回跳跃，反而稳定性较好。
...
因为在自然环境里大部分情况下，流体密度都不会发生多大变化，所以我们这里只研究不可压缩流体，可压缩流体不在讨论范围内。在不可压缩流体模拟中，我们希望粒子 $i$ 的密度尽量与静止密度 $\rho_0$ 相同，即 $\rho_i = \rho_0$ 。因此需要针对每一个流体粒子都施加一个常量密度约束，PBF 将该约束定义为：
$$
...
$$
我们用 $\vec{x}_k$ 表示受约束 $C_i$ 约束的粒子，即：
$$
...
自此，PBF求解部分已经完成了。最后还有个技巧避免不符合物理的问题。
