<center><font size="36" family="微软雅黑"> 基于物理的交互式流体模拟介绍</font></center>
...
那是不是这样子求压力梯度就符合物理原理没问题呢？现在，我们来看两个相邻的粒子，如图：
...
于是，有人提出了另外一种方案，这方案不需要像前述所说的求解力，而是基于位置的动力学，英文叫 Position Based Dynamics ，简称 PBD。
...
<img src="ppt image/PBD-velocity-correction.png" alt="PBD-velocity-correction" style="zoom:48%;" />
...
<img src="ppt image/PBD-Lagrange-Multiplier.png" alt="PBD-Lagrange-Multiplier" style="zoom:48%;" />
...
$$
另外前面也提到不管是 $\pmb{p}$ 还是 $\pmb{p}+\pmb{\Delta{p}}$ 都要满足约束，所以 $C(\pmb{p})=0$ 和 $C(\pmb{p}+\Delta \pmb{p})=0$ 是等价的，而现在我们要求的是 $\Delta \pmb{p}$ ，所以我们用 $C(\pmb{p}+\Delta \pmb{p})=0$ 代替 $C(\pmb{p})=0$ 。现在有两个未知数 $\pmb{\Delta{p}}$ 和 $\lambda$ ，两个方程，可以解方程组了。联立方程组：
$$
\begin{equation}
\left\{
\begin{aligned}
...
<img src="ppt image/PBD-constraint-solution.png" alt="PBD-constraint-solution" style="zoom:48%;" />
...
<img src="ppt image/PBD-Gauss-Seidel.png" alt="PBD-Gauss-Seidel" style="zoom:48%;" />

<center>高斯-赛德尔迭代求解过程</center>

使用高斯-赛德尔方法求解，第一步先代入 约束 $C_1$ ，解会在 $C_1$ 的解空间 $l_1$ 中。然后把解代入约束 $C_2$ ，解会在约束 $C_2$ 的解空间 $l_2$ 中。也就是说用高斯-赛德尔方法解会在解空间之间来回跳跃，然后慢慢靠近共同的解空间。整体来讲，高斯-赛德尔迭代收敛速度较快，并且由于每一个约束方程求解都需要用上一个约束方程的解作为当前约束方程的输入，所以导致无法并行。

我们再看看高斯-赛德尔的伪代码，更加直观。

```c++
int N; // 粒子数
int M; // 约束数
Matrix<N,N> InvM; // 所有粒子质量倒数构成的质量对角矩阵
float lambdas[M]; // 拉格朗日乘子数组
Vector3 deltaPos[N]; // 约束产生的修正位移
Vector3 pos[N];
for (int i = 0; i < M; i++)
{
    // 求解第 i 个约束
	deltaPos = lambdas[i] * InvM * constraint_gradient(pos);
    // 根据修正位移，更新所有粒子位置
	update_position(pos, deltaPos);
}
```

然后我们看看**雅可比迭代**是怎么求解的。如图所示：

<img src="ppt image/PBD-Jacobi.png" alt="PBD-Jacobi" style="zoom:48%;" />

<center>雅可比迭代求解过程</center>

雅可比迭代先求解关于约束 $C_1$ 的方程，计算出到 $C_1$ 解空间 $l_1$ 的向量。然后求解关于约束 $C_2$ 的方程到 $C_2$ 在解空间 $l_2$ 的向量。然后向着两个向量的合向量前进，寻找关于 $C_1$ 、$C_2$ 的相交解空间。这样的话如果 $C_1$ 和 $C_2$ 的解空间在同一个方向，雅可比迭代使用合向量作为步长，经常会一步迈过，下一步再买回起始点，导致不能收敛。但是当解空间不同向的时候，雅可比迭代法由于向着合向量迭代，不会像高斯-赛德尔方法那样在两个解空间之间来回跳跃，反而稳定性较好。我们看看代码，更加直观点。
...
Matrix<N,N> InvM; // 所有粒子质量倒数构成的质量对角矩阵
float lambdas[M]; // 拉格朗日乘子数组
Vector3 deltaPos[M][N]; // 约束产生的修正位移
Vector3 pos[N];
for (int i = 0; m < i; i++)
{
    // 求解第 i 个约束
	deltaPos[i] = lambdas[i] * InvM * constraint_gradient(pos);
}
for (int i = 0; i < M; i++)
{
    // 根据修正位移，更新所有粒子位置
	update_position(pos, deltaPos[i]);
}
```

从这里我们可以看到雅可比迭代收敛速度慢甚至可能无法收敛，但是因为其解之间并无依赖，所以利于**并行**。

我们列个表从收敛性和可并行性以及时间复杂度来更直观比较：

|     迭代法      | 收敛速度 | 可并行性 | 时间复杂度 |
| :-------------: | :------: | :------: | :--------: |
|   雅克比迭代    |    慢    |   可以   |  $O(N^2)$  |
| 高斯-赛德尔迭代 |    快    |  不可以  |  $O(N^2)$  |

那么 PBD 选择用哪一种迭代方式呢？由于雅可比迭代可以并行，在计算机中更方便使用。另外为了解决上面提到的一步迈过不能收敛的问题，有人提出使用平均雅可比迭代法，就是把合向量接起来，然后除以向量个数。 PBD 也是采用了这种方式。另外为了加速收敛，PBD 还加上了超松弛因子 $\omega$ 。不管哪一种方法，都是加速其收敛，同时也不管哪种方法，其解都是近似解，而不是精确解。

### 约束求解优先级

在实践中，不同约束类型之间的处理是有优先级的，为了实现这个，我们按照约束类型分组，相同约束类型的分成一组。然后优先级高的约束分组先处理，即先求出 $\Delta \vec{p}_i$ 并把该值累加到原位移 $\vec{p}_i$ 上，然后再处理其他优先级较低的约束分组。例如，先处理碰撞约束，让粒子跟物体不发生接触，把该分离后的位置应用到原位移上，得出新位置 $\vec{p}_i^*$ ，然后再处理流体密度约束。这种方式能加快约束修正位置的收敛速度，能够快速接近真实解。

至此，这就是 PBD 算法的来龙去脉，PBD 的求解其实也应用了方程组迭代求解方法。当然，具体的约束根据不同形态的物体，应用不一样的约束条件。接下来我们看看流体相关的约束及其处理。

# 基于位置流体模拟（Position Based Fluid）

前面讲了 PBD 算法的原理、过程的推导和求解方法。那么我们基于 PBD 的流体模拟是怎么实现的呢？下面我们来讨论下基于位置的流体模拟 —— PBF。在 PBD 中，主要有这么几个问题是：

- 确定模拟的物体是什么，根据模拟物体选择合适的约束函数 $C$ 。
- 按照拉格朗日乘子法，求解拉格朗日乘子 $\lambda$ 
- 根据约束优化，最后求解出修正位置 $\Delta{\pmb{p}}$

因此，基于 PBD 出来的 PBF 主要的问题也集中在上面三个。下面我们逐个问题来寻找答案。

## 流体的密度约束

因为在自然环境里大部分情况下，流体密度都不会发生多大变化，所以我们这里只研究不可压缩流体，可压缩流体不在讨论范围内。在不可压缩流体模拟中，我们希望粒子 $i$ 的密度尽量与静止密度 $\rho_0$ 相同，即 $\rho_i = \rho_0$ 。因此需要针对**每一个流体粒子**都施加一个常量密度约束，PBF 将该约束定义为：
$$
C_i(\vec{p}_1, \cdots, \vec{p}_n) = \frac{\rho_i}{\rho_0} - 1
\label{eq:PBF-Density-Equal-Constraint}
$$
从这里可以看到，我们要求密度约束，我们已经知道静止密度 $\rho_0$ ，所以剩下就是求粒子 $i$ 的密度 $\rho_i$ 。这里我们使用之前介绍过的 SPH 计算物理量的算法。前面在 SPH 那一节我们提到如何求空间位置 $\vec{x}_i$  一个物理量：
$$
A_i = \sum_j \frac{m_j}{\rho_j} A_j W(\vec{x}_i - \vec{x}_j, h)
$$
我们再回顾下这个公式，其中 $j$ 表示所有邻居粒子，$W(\vec{x}_i-\vec{x}_j, h)$ 是核函数，就是求权重的，h 是核半径，$\vec{x}_i - \vec{x}_j$ 就是两个粒子之间的距离了。

有了这个公式之后，既然 $A_i$ 是物理量，而密度也是一种物理量，所以可以把密度 $\rho$ 直接代入该公式，可以得出粒子 $i$ 的密度是：
$$
\rho_i = \sum_j m_j W(\vec{x}_i - \vec{x}_j, h)
$$
由于流体每个粒子质量相等，也就是质量所起的权重作用是相同的，所以在这里我们可以省去质量 $m_j$ 。
$$
\rho_i = \sum_j W(\vec{x}_i - \vec{x}_j, h)
\label{eq:PBF-Density-KDE}
$$

我们看下密度计算的实际代码，会更加直观。

```c++
double DensityConstraint::computeDensity(vector<Particle*> particles, int i)
{
    Particle *pi = particles[i]; // 粒子 i
    for (int j = 0; j < neighbors[i].size(); j++)
    {
        int neighborIndex = neighbors[i][j];
        // 邻居粒子 j
        Particle *pj = particles[neighborIndex];
        // 两个粒子距离
        Vector3 dist = pi->pos - pj->pos;
        // 权重求和计算密度
        pi->density += kernel.W(dist);
    }
}
```

按照前述 PBD 的算法，需要用到约束 $C_i$ 的梯度。把公式$\eqref{eq:PBF-Density-KDE}$ 代入公式 $\eqref{eq:PBF-Density-Equal-Constraint}$ ，可得：
$$
C_i(\vec{p}_1, \cdots, \vec{p}_n) = \frac{\rho_i}{\rho_0} - 1 = \frac{1}{\rho_0} \sum_j W(\vec{x}_i - \vec{x}_j, h) - 1
\label{eq:PBF-Density-Equal-Constraint-Extension}
$$
我们用 $\vec{x}_k$ 表示受约束 $C_i$ 约束的粒子位置，即：
$$
C_i(\vec{p}_1, \cdots, \vec{p}_n) = \frac{\rho_i}{\rho_0} - 1 = \frac{1}{\rho_0} \sum_k W(\vec{x}_i - \vec{x}_k, h) - 1
\label{eq:PBF-Density-Equal-Constraint-Extension-k}
$$
那么对 $\vec{x}_k$ 求导就是梯度。那这个梯度怎么算？我们可以使用之前 SPH 提到的梯度估算来近似计算物理量的梯度，即 ：
$$
\nabla A_i = \Sigma _j \frac{m_j}{\rho_j}A_j \nabla W_{ij}
$$
这里由于流体粒子质量相同，所以这里可以化简成：
$$
\nabla A_i = \Sigma _j A_j \nabla W_{ij}
$$
我们用这个估算，去计算 $\nabla_{\vec{x}_k} C_i$ ，所以这里梯度是：
$$
\nabla_{\vec{x}_k} C_i = \frac{1}{\rho_0} \sum_j \nabla_{\vec{x}_k} W(\vec{x}_i - \vec{x}_j, h)
\label{eq:PBF-Density-Constraint-Gradient}
$$
$\nabla_{\vec{x}_k}{C_i}$ 表示约束函数 $C_i$ 关于 $\vec{x}_k$ 的梯度。也就是粒子 $i$ 的约束函数对 $\vec{x}_k$ 求梯度。注意，这里的对 $\vec{x}_k$ ， 而不是 $\vec{x}_j$ ，这表明不仅仅是邻居粒子 $j$ 这么简单了。由于自己的梯度无法直接求得，需要使用 KDE 通过邻域内粒子的梯度权重求和得到，所以这里要分两种情况：粒子 $k$ 为粒子 $i$ 自身（$k=i$）或粒子 $i$ 的邻居粒子 $j$ （$k=j$），即：
$$
\nabla_{\vec{x}_k} C_i = \frac{1}{\rho_0}
\begin{cases}
\sum_j \nabla_{\vec{x}_k} W(\vec{x}_i - \vec{x}_j, h) \quad &k = i \\
- \nabla_{\vec{x}_k} W(\vec{x}_i - \vec{x}_j, h) \quad &k = j
\end{cases}
\label{eq:PBF-Density-Constraint-Gradient-Cases}
$$
如图所示：

<img src="ppt image/PBF-density-constraint-gradient.png" alt="PBF-density-constraint-gradient" style="zoom:48%;" />

<center>密度约束的梯度方向</center>

我们可以这么理解：

- 当 $k=i$ 时，$\nabla_{\vec{x}_k} C_i = \nabla_{\vec{x}_i}C_i$ 表示约束函数 $C_i$ 关于 $\vec{x}_i$ 的梯度，方向为 $\frac{\vec{x}_i-\vec{x}_j}{\parallel \vec{x}_i - \vec{x}_j\parallel} $ ，其值通过邻域粒子的梯度权重求和得到
- 当 $k=j$ 时，$\nabla_{\vec{x}_k} C_i = \nabla_{\vec{x}_j}C_i$ 表示约束函数 $C_i$ 关于 $\vec{x}_j$ 的梯度，方向为 $-\frac{\vec{x}_i-\vec{x}_j}{\parallel \vec{x}_i - \vec{x}_j\parallel}$

也许这么说还是比较绕，还是上代码片段，更加直观。

```c++
Vector3 DensityConstraint::gradient(vector<Particle*> particles, int i, int k)
{
    Vector3 grad;
	Particle *pi = particles[i], *pk = particles[k];
    if (pk == pi)
    {
		// k == i，通过邻居梯度求和近似自己梯度
		for (int x = 0; x < neighbors[i].size(); x++)
		{
			Particle *pj = particles[x];
			Vector3 dist = pi->pos - pj->pos;
    ...
    {
        // k == j，直接用核函数梯度计算邻居梯度
        Vector3 dist = pi->pos - pk->pos;
        grad = -kernel.Gradient(dist);
    }
    return grad / density0;
}
```

到这里，我们得出了流体的密度约束函数及其函数相关的物理量的求解。
...
\label{eq:PBF-Density-Constraint-Lagrange-Multiplier}
$$
可能很多人看到各种下标 $i$ ，$j$ 等都看糊涂了。这里补充说明一下。按照 PBD 算法，一个约束对应一个拉格朗日乘子 $\lambda$ ，而这里流体约束是一个粒子只受一个约束，所以这里 $\lambda_i$ 下标刚好跟粒子 $i$ 对应上。而实际上 PBD 中的 $\lambda$ 数量可以等于粒子数，也可以不等于粒子数的。只是在流体中，模拟的物体的 N 个粒子和 M 个约束中的 N 和 M 刚好相等。

回到求解 $\lambda$ 问题上，这样直接计算拉格朗日乘子 $\lambda$ 是否就能求解出来呢？实际上有可能求解不出来。那是什么问题呢？

如果一个约束条件不能被违反，则称为硬约束；反之，能够一定程度上被违反的约束为软约束。理想情况下，我们当然希望约束始终都是硬约束，但是由于计算误差或者数值稳定性等原因，我们有时也需要约束呈现出软性质。

<img src="ppt image/PBF-lagrange-multiplier.png" alt="PBF-lagrange-multiplier" style="zoom:48%;" />

<center>邻居粒子 j 处于粒子 i 的光滑核半径 h 边缘时</center>

在 PBF 中，我们把粒子 $i$ 和粒子 $j$ 的距离记作 $\vec{r} = \vec{x}_i - \vec{x}_j$ ，当 $\parallel \vec{r} \parallel = h$ 时，即邻居粒子 $j$  处于粒子 $i$ 的光滑核半径 $h$ 边缘时（此时粒子 $i$ 和粒子 $j$ 处于分离状态），则前述的核函数梯度值 $\nabla W(\vec{r}, h) = 0$ 。如果所有邻域粒子之间都处于这种状态，由前述的 PBF 梯度公式 $\eqref{eq:PBF-Density-Constraint-Gradient-Cases}$ 可知：
$$
\begin{aligned}
...
\end{aligned}
$$
从而公式 $\eqref{eq:PBF-Density-Constraint-Lagrange-Multiplier}$ 分母 $\sum_k \parallel \nabla_{\vec{p}_k} C_i \parallel^2 = 0$ ，这会导致除零错误。为了解决这个问题，PBF 借鉴了 Open Dynamics Engine 中混合约束力方法，使密度约束变成软约束。具体做法就是直接加入一个松弛参数 $\epsilon$ ，这个由用户直接指定。则拉格朗日乘子变成：
$$
\lambda_i = - \frac{C_i(\vec{p}_i, \cdots, \vec{p}_n)}{\sum_k \parallel \nabla_{\vec{p}_k} C_i\parallel^2 + \epsilon}
$$

我们再看看代码是如何实现拉格朗日乘子的，仍然以 C++ 为例：

```c++
void DensityConstraint::computeLagrangeMultiplier(vector<Particle*> particles, int i)
{
    Particle *pi = particles[i]; // 粒子 i
    double C = pi->density / density0 - 1, 0.0;	// 密度约束
    if (C != 0.0)
    {
        double sum_of_grad_len2 = 0.0; // 保存梯度长度平方和
        for (int j = 0; j < neighbors[i].size(); k++)
        {
            // 邻居粒子 j
            Particle *pj = particles[neighbors[i][j]];
            Vector3 grad = gradient(particles, i, k);
            sum_of_grad_len2 += grad.squareLength();
        }
        // 自己的梯度也需要考虑进去
        Vector3 grad = gradient(particles, i, i);
        denom += grad.squareLength();
        // 计算拉格朗日乘子
        pi->lambda = -C / (sum_of_grad_len2 + epsilon);
    }
    else
    {
        pi->lambda = 0.0;
...
有了拉格朗日乘子 $\lambda$ ，我们可以求解最终的粒子 $i$ 的修正位移为：
$$
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
\label{eq:PBF-Density-Constraint-Delta_Position}
$$
...
```c++
Vector3 DensityConstriant::computeDeltaPosition(vector<Particle*> particles, int i)
{
    Vector3 deltaPos;
    double invDensity0 = 1 / density0;
    Particle *pi = particles[i]; // 粒子 i
    for (int j = 0; j < neighbors[i].size(); j++)
    {
        int neighborIndex = neighbors[i][j];
        Particle *pj = particles[neighborIndex]; // 邻居粒子 j
        Vector3 dist = pi->pos - pj->pos; // 两个粒子的距离
        Vector3 grad = gradient(dist); // 梯度
        double lambda = (pi->lambda - pj->lambda); // lambda_i + lambda_j
        deltaPos += lambda * grad; // lambda * gradient
    }
    deltaPos *= invDensity0; // sum((lambda_i + lambda_j) * gradient) / density0
    return deltaPos;
}
```

至此，PBF求解部分已经完成了。最后还有个技巧避免不符合物理的问题。

## Tensile Instability

对于采用 SPH 插值技术计算密度的流体模拟方法，通常需要30∼40个邻居粒子才能使密度求值结果趋于静态密度。在邻居粒子不足的情况下，会导致求出的流体密度低于静态密度，由此造成压强为负数，原本粒子间的压力变为吸引力，使粒子产生不符合实际情况的凝聚，此即 SPH 的 Tensile Instability 问题在流体模拟中的具体体现，这导致的结果是流体表面的模拟给人感觉不真实。如图所示：

<img src="ppt image/PBF-tensile-instability.png" alt="PBF-tensile-instability" style="zoom:75%;" />

<center>负压导致的 Tensile Instability 问题</center>
...
- 等式约束：总是进行投影操作。
- 不等式约束 ：只有在不等式约束条件不满足，即 $C_i(\vec{x}_1, \cdots, \vec{x}_n)<0$ 时才进行约束投影操作。

<img src="ppt image/PBF-tensile-instability-solution.png" alt="PBF-tensile-instability-solution" style="zoom:75%;" />

<center>不等式流体密度约束</center>

如图上图所示，只有在上面的单边约束条件不满足，即 $\frac{ρ_i}{ρ_0} -1>0$ 或 $ρ_i>ρ_0$ 时，才进行约束投影操作。直观理解就是只有在粒子靠的比较近（流体压缩了）的情况下，才需要进行操作让粒子分开（保持流体不可压缩）。而当 $\frac{ρ_i}{ρ_0} -1≤0$ 或 $ρ_i≤ρ_0$ 不等式约束条件满足，此时不需要进行约束投影，因此也就避免了表面粒子凝聚问题。

这两种方法根据具体情况来选择。要更加真实的效果，可以选择第一种方法，添加排斥力。如果要求没有那么高，可以选择第二种不等约束的方法，在 NVIDIA 的 FleX 中是采用了第二种方法来避免 Tensile Instability 问题的出现。

这样子，我们上面计算拉格朗日的代码需要稍微修改下，代码片段如下：

```c++
void DensityConstraint::computeLagrangeMultiplier(vector<Particle*> particles, int i)
{
	...
    double C = std::max(pi->density / density0 - 1, 0.0);	// 密度约束
    ...
}
```
...
其实基于 PBD 能做很多事情，例如把整个物理世界用粒子来模拟，那么可以把刚体、软体、沙粒、布料、流体等统一起来，实现各种不同材料物体之间的交互。只要把各种约束实现，就能实现不同类型物质的模拟。

并行计算，这个跟计算机落地实现有关，真是的模拟上在 PC 等 GPU 强的硬件环境中，是比较容易做到的，但是在终端等 GPU 计算能力比较弱的设备上，还需要进一步研究探索并行化。当然，obi 依赖 burst 和 job system 做到了终端上有限粒子的物理模拟。

还有流体力学中的涡流、湍流等，在实时模拟中也是计算量比较大的，也是比较难以实现的。这方面也可以进一步探索。

最后这里也没有讨论流体的渲染，这里顺带一句，流体可以用基于屏幕空间的流体渲染来实现，具体的原理和实现，可以自行去进一步了解。

# 参考文献

[Müller, Matthias, et al. "Position based dynamics." Journal of Visual Communication and Image Representation 18.2 (2007): 109-118.](https://dl.acm.org/doi/10.1016/j.jvcir.2007.01.005)

[Macklin, Miles, and Matthias Müller. "Position based fluids." ACM Transactions on Graphics (TOG) 32.4 (2013): 1-12.](https://dl.acm.org/doi/10.1145/2461912.2461984)

[Macklin, Miles, et al. "Unified particle physics for real-time applications." ACM Transactions on Graphics (TOG) 33.4 (2014): 1-12.](https://dl.acm.org/doi/10.1145/2601097.2601152)

https://zhuanlan.zhihu.com/p/48737753

https://zhuanlan.zhihu.com/p/49536480

《计算流体力学入基础及其应用》（美）约翰 D. 安德森 (John D. Anderson) 著  吴颂平 刘赵淼译
